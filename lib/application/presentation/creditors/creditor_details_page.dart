...
class CreditorDetailsPage extends StatelessWidget {
  ...
  CreditorDetailsPage({super.key, required this.creditor});

  @override
  Widget build(BuildContext context) {
    final CreditorDetailsController controller =
        Get.put(CreditorDetailsController(creditorId: creditor.id));
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            CustomRowWidget('Name', creditor.name),
            CustomRowWidget('Phone Number', creditor.phoneNumber.toString()),
            creditor.address == ''
                ? const SizedBox()
                : CustomRowWidget('Address', creditor.address),
            CustomRowWidget('Total Outstanding Credit',
                creditor.totalOutstandingCredit.toString()),
